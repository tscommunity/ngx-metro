...
import { MetroFormGroup } from '../../form/metro-form-group';
import { MetroDialogDataEmitter, MetroDialogDataInput } from './dialog-interfaces';
...
export class DialogService {
   ...
   */
  public formPrompt<T = string>(title: string, form: MetroFormGroup, submitBtnText?: string, abortBtnText?: string, placeholder?: string,
   ...
   */
  public show<TOutput = null>(component: new (...args: any[]) => any, dialogData?: any, title?: string, closeBtnText?: string, cls?: string,
                              closeBtnCls?: string): Observable<TOutput> {
    const componentRef: ComponentRef<any> = this.componentFactoryResolver.resolveComponentFactory(component).create(this.injector);

    if (!!dialogData) {
      (<MetroDialogDataInput<TOutput>>componentRef.instance).dialogDataInput = dialogData;
    }
    ...
    if (!!(<MetroDialogDataEmitter<TOutput>>componentRef.instance).dialogDataEmitter) {
      dialogDataSubscription = (<MetroDialogDataEmitter<TOutput>>componentRef.instance).dialogDataEmitter
        .pipe(finalize(() => {
          subject$.complete();
        })).subscribe((data) => {
          subject$.next(data);
